...
- (id)initWithSize:(CGSize)size {
    if (self = [super initWithSize:size]) {
        ...
        int maxObstacles = UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ? 25 : 20;
        NSMutableArray *gameCharacters = [NSMutableArray arrayWithCapacity:maxObstacles];
        ...
        for (int i = 0; i < 3; ++i) {
            [_gameCharacters addObject:[[ABSeedBag alloc] initAtPosition:CGPointZero]];
        }
        
        _starField = [[ABStarField alloc] initWithSize:self.size];
        _starField.name = @"starField";
        [self addNode:_starField atWorldLayer:ABWorldLayerStars];
...
- (void)spawnObject
{
    ...
    if (abRand(0, 1) > 0.97) {
        ...
        for (ABSeedBag *bag in _gameCharacters) {
            if ([bag isKindOfClass:[ABSeedBag class]] && !bag.characterScene) {
                [bag reset];
                [bag addToScene:self atPosition:CGPointMake(self.size.width + bag.size.width,
...
- (void)update:(NSTimeInterval)currentTime {
    ...
    for (ABGameObstacle *gameCharacter in _gameCharacters) {
        
        [gameCharacter update:delta];
        
        if (_gameStarted) {
            if (gameCharacter.characterScene == self) {
                if (!gameCharacter.hasPassedPlayerCharacter) {
                    BOOL passedPlayer = gameCharacter.position.x + gameCharacter.size.width / 2 <
                    _playerCharacter.position.x - _playerCharacter.size.width / 2;
                    if (passedPlayer) {
                        [gameCharacter setPassedPlayerCharacter:YES];
                        if (_gameStarted) {
